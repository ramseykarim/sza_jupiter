...
class Plotting:
    ...
    def add_model_plot(self, root_name, color, line_sty='-', label=None, linewidth=2.0):
        ...
        self.labels_list_models.append(label)
        if color.lower() == "default":
            plt.plot(self.f_or_w(frequencies), points, line_sty)  # , linewidth=linewidth)
        else:
            plt.plot(self.f_or_w(frequencies), points, line_sty, color=color, linewidth=linewidth)
